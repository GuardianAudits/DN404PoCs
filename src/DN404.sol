...
abstract contract DN404 {
                ...
                if (packedLogs.logs.length != 0) {
                    ...
                    do {
                        ...
                        if (burnedPoolHead != t.burnedPoolTail) {
                            ...
                            console.log("Using burn pool", id);
                        } else {
                            id = t.nextTokenId;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        unchecked {
            ...
            console.log("numBurns", t.numNFTBurns);
            console.log("numMints", t.numNFTMints);
                ...
                do {
                    ...
                    if (burnedPoolHead != t.burnedPoolTail) {
                        ...
                        while (_get(oo, _ownershipIndex(id)) != 0) {
                            id = _useExistsLookup()
                                ? _wrapNFTId(_findFirstUnset($.exists, id + 1, maxId), maxId)
                                : _wrapNFTId(id + 1, maxId); // @audit large gas expenditure here if loops a lot
                        }
                        t.nextTokenId = _wrapNFTId(id + 1, maxId);
                    }
                    if (_useExistsLookup()) _set($.exists, id, true);
        ...
        virtual
    {
        ...
        if (from != $.aliasToAddress[_get(oo, _ownershipIndex(_restrictNFTId(id)))]) { // @audit odd that they restrict for validation
